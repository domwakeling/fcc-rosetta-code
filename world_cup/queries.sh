...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams AS t LEFT JOIN games AS w ON t.team_id = w.winner_id LEFT JOIN games AS o ON t.team_id = o.opponent_id WHERE (o.year=2014 AND o.round='Eighth-Final') OR (w.year=2014 AND w.round='Eighth-Final') ORDER BY name ")"
